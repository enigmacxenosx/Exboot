...
The current application version is `0.3.8`. Publish future builds with semantic-style tags such as `v0.3.8` or `v0.4.0` so the checker can compare them correctly. The installer and executable use the Enosx AI splash logo from `assets/enosx-ai-splash-logo.ico`, with matching wizard artwork from `assets/enosx-ai-splash-wizard.png`.

## Version 0.3.8

Version 0.3.8 enables the dark theme by default for new installations and for settings files that do not yet contain a theme preference. Existing users keep their saved appearance choice, and the Appearance dialog continues to provide the dark-mode toggle. The scrollable main interface now initializes its canvas background consistently with the selected theme.

## Version 0.3.7

Version 0.3.7 adds a vertically scrollable main interface so lower controls remain accessible on smaller screens and after switching between single-ISO and multi-boot modes. The scroll region resizes with the window, supports the mouse wheel, and is covered by the headless GUI smoke test.

## Version 0.3.6

Version 0.3.6 keeps PowerShell, DiskPart, DISM, and registry helper processes hidden behind the standalone Tkinter window. Completing installation now launches the windowed `Exboot.exe` without exposing a console window.

## Version 0.3.5

Version 0.3.5 fixes the Windows CI installation smoke test by installing into a controlled temporary directory before launching the installed application with `--smoke-test`. The release workflow now verifies the actual packaged installer instead of only testing the source checkout.

## Version 0.3.4

Version 0.3.4 adds a Windows CI installation smoke test. Each tagged build silently installs the generated setup executable and launches the installed application with `--smoke-test`, so the release is checked for the startup crash before the installer is published.

## Version 0.3.3

Version 0.3.3 fixes a Windows startup crash caused by stale or partially-created Tkinter media-mode frames. The startup and mode-switch paths now tolerate missing or destroyed widgets instead of raising an invalid `disk_frame` command error.

## Version 0.3.2

Version 0.3.2 adds a safe automatic update workflow. Exboot checks GitHub Releases, downloads only the matching single-file setup executable after confirmation, verifies its SHA-256 asset digest, and closes before launching the installer.
...
The installer will be created as the single setup executable `installer-output\\ExbootSetup-0.3.8.exe`. It uses the Enosx AI logo as its Windows file icon, installs Exboot under Program Files, creates a Start Menu shortcut, optionally creates a desktop shortcut, and can launch Exboot after installation. GitHub publishes the installer asset’s SHA-256 digest, which the automatic updater uses to verify downloads.

## Automated release builds

Pushing a version tag such as `v0.3.8` starts the Windows GitHub Actions build. The workflow builds `Exboot.exe`, installs Inno Setup on the Windows runner, creates the single installer executable, stores it as a workflow artifact, and attaches only that `.exe` to the matching GitHub Release. The repository must have Actions enabled and permission to write release contents.
